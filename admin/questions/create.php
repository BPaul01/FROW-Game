...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if(empty($fruit_id_err) && empty($level_err) && empty($question_err) && empty($photo_err))
    {
        ...
        $stmt = $db->getDb()->prepare("INSERT INTO questions (`fruit_id`, `level`, `question`, `photo`) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("isss", $fruit_id, $level, $question, $imagename);
        if($stmt->execute()) {
            $success = true;
            $imagePath = $_SERVER["DOCUMENT_ROOT"] . "/assets/images/questions/{$stmt->insert_id}/";
